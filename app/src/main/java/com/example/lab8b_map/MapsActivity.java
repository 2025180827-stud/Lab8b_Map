...
import com.example.lab8b_map.databinding.ActivityMapsBinding;
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, url, null, response -> {
                    try {
                        ...
                        for (int i = 0; i < Math.min(results.length(), 10); i++) {
                            ...
                            Marker marker = mMap.addMarker(new MarkerOptions().position(latLng).title(name).icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));
